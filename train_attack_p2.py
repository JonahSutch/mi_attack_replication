...
Example:
    python train_attack_p2.py --train_size 2500
    python train_attack_p2.py --train_size 10000

The SLURM array job (slurm/train_attack_p2.sh) calls this once per size.

Output layout:
    results/attack_models_p2/
...
def main():
    ...
    parser.add_argument('--save_dir',    type=str, default='./results/attack_models_p2')
    ...
    if not os.path.exists(attack_data_path):
        raise FileNotFoundError(
            ...
            f"Run train_shadows_p2.sh + merge_shadows_p2.sh for size={args.train_size} first."
        )
